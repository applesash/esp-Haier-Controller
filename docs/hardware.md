...
- RS485 uses automatic transceiver direction; ESP32 UART pins are TX=17 and
	RX=18. A/B are transceiver terminal signals, not ESP32 GPIOs.
...
Current status: the 28141 BOX identity, 800x480 display size, automatic RS485
direction, and CH422G digital I/O channel names are documented. Physical
electrical verification and exact display/touch controller identification remain
open.
...
- Display controller identification (panel IC or known Waveshare design variant).
- Touch controller and GPIO mapping for reset, interrupt, backlight, and power rails.
...
Until the remaining evidence exists, the display driver and field-bus connection
must remain in their safe bring-up modes. The firmware CH422G probe is read-only,
the RS485 observer never transmits, and commissioning/OTA write endpoints return
locked responses.
