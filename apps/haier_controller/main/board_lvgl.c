...
static void flush_callback(lv_disp_drv_t *drv, const lv_area_t *area, lv_color_t *color_map)
{
    ...
    if (lv_disp_flush_is_last(drv)) {
        ...
        if (drv->full_refresh) {
            ...
            static bool active_flush_logged;
            static bool flush_logged;
            ...
            flush_get_next_buf(panel_handle);
                    if (!active_flush_logged) {
                        active_flush_logged = true;
                        ESP_LOGI(TAG, "Active LVGL flush: %d,%d to %d,%d", offsetx1, offsety1,
                                 offsetx2, offsety2);
                    }
        } else {
...
static void flush_callback(lv_disp_drv_t *drv, const lv_area_t *area, lv_color_t *color_map)
{
    ...
    const int offsetx2 = area->x2; // End X coordinate of the area to flush
            if (!flush_logged) {
                flush_logged = true;
                ESP_LOGI(TAG, "First LVGL flush: %dx%d..%dx%d", offsetx1, offsety1,
                         offsetx2, offsety2);
            }
    const int offsety1 = area->y1; // Start Y coordinate of the area to flush
    const int offsety2 = area->y2; // End Y coordinate of the area to flush
...
{
    static bool first_flush_logged;
    esp_lcd_panel_handle_t panel_handle = (esp_lcd_panel_handle_t) drv->user_data; // Get the panel handle from driver user data
    const int offsetx1 = area->x1; // Start X coordinate of the area to flush
    const int offsetx2 = area->x2; // End X coordinate of the area to flush
    const int offsety1 = area->y1; // Start Y coordinate of the area to flush
    ...
    if (!first_flush_logged) {
        ...
        ESP_LOGI(TAG, "First LVGL flush: %d,%d to %d,%d", offsetx1, offsety1,
                 offsetx2, offsety2);
    }
...
{
    ESP_LOGI(TAG, "LVGL task entered");

    uint32_t task_delay_ms = LVGL_PORT_TASK_MAX_DELAY_MS; // Set initial task delay
    bool first_handler_logged = false;
    while (1) {
        if (lvgl_port_lock(-1)) { // Try to lock LVGL mutex
            task_delay_ms = lv_timer_handler(); // Handle LVGL timer events
            if (!first_handler_logged) {
                first_handler_logged = true;
                ESP_LOGI(TAG, "LVGL timer loop active; next delay=%lu ms",
...
esp_err_t board_lvgl_init(esp_lcd_panel_handle_t lcd_handle)
{
